...
int minimalHit = 700; //value for a kick
int maxHit = 900; //value for a punch
...
String player = "player2"; //Change this to either "player1" or "player2"
int score = 0;

if (sensorValue0 < minimalHit < maxHit < 1000 || sensorValue1 < minimalHit < maxHit < 1000 || sensorValue2 < minimalHit < maxHit < 1000 || sensorValue3 < minimalHit < maxHit < 1000) {
  score = 2;
}
else {
  score = 1;
...
if (client.connect(serv, 80) ==1 && (sensorValue0 < minimalHit || sensorValue1 < minimalHit || sensorValue2 < minimalHit || sensorValue3 < minimalHit)) { //Connecting at the IP address and port we saved before
  ...
  delay(1500); //Change this if consecutive hits are not being registered
  }
else {
